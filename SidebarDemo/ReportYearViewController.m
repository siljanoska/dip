...
@interface ReportYearViewController ()

@property (strong) NSArray *objectCat;
@end

@implementation ReportYearViewController
...
- (void)viewDidLoad
{
    ...
    [dateFormater setDateFormat:@"yyy"];
    _dateString = [ dateFormater stringFromDate: currentDate];
    _dateFromPicker = currentDate;
    [Globals saveDate:_dateString];
    [_dateYearButton setTitle:_dateString forState:UIControlStateNormal];
    
    _dateYearButton.titleLabel.text =_dateString;
    brojac =0;
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    if([segue.identifier isEqualToString:@"tabCustom"]){
