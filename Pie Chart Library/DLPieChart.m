...
//        }
        ...
        [_pieView setBackgroundColor:[UIColor clearColor]];
        [_pieView setBackgroundColor:[UIColor redColor]];
...
- (SliceLayer *)createSliceLayer
{
    ...
    CGSize size = [@"" sizeWithFont:self.labelFont];
    [CATransaction setDisableActions:YES];
...
- (void)renderInLayer1:(DLPieChart *)layerHostingView dataArray:(NSMutableArray *)dataArray nameArray:(NSMutableArray*)nameArray{
    ...
    [layerHostingView setLabelFont:[UIFont fontWithName:@"DBLCDTempBlack" size:10.0]];//(24-DLDataArray.count)]];
   ...
   [self drawLegends:layerHostingView dataArray:nameArray];
}


-(void)drawLegends:(DLPieChart *)layerHostingView dataArray:(NSMutableArray*)dataArray
{
    for (UIView *subView in [layerHostingView subviews]) {
    ...
    legendX = layerHostingView.pieCenter.x+layerHostingView.pieRadius+OFFSET;
    legendY = 30;
    ...
    legendsScrollView.frame = CGRectMake(legendX+10, legendY, legendWidth,legendHeight);
    ...
    for (int i=0;i<dataArray.count;i++)
    {
        ...
        btn.frame = CGRectMake(5, y, 20, 20);
        btn.tag = i;
        ...
        UILabel *lbl = [[UILabel alloc] initWithFrame:CGRectMake(30, y, 100, 25)];
    ...
    [layerHostingView addSubview:legendsScrollView];
}
...
-(void)drawLegends1:(DLPieChart *)layerHostingView dataArray:(NSMutableArray*)dataArray
{
    ...
    for (int i=0;i<dataArray.count;i++)
    {
        ...
        UILabel *lbl = [[UILabel alloc] initWithFrame:CGRectMake(10, y, legendWidth-OFFSET*2, 25)];
        lbl.text = [NSString stringWithFormat:@"%@",[dataArray objectAtIndex:i]];
        lbl.backgroundColor = [UIColor clearColor];
        [lbl setAdjustsFontSizeToFitWidth:YES];
        [legendsScrollView addSubview:lbl];
...
//- (void)pieChart:(DLPieChart *)pieChart didSelectSliceAtIndex:(NSUInteger)index dataArrat:(NSArray*)dataArray
//{
//    NSLog(@"did select slice at index %lu",(unsigned long)index);
//    UILabel *lab = [[UILabel alloc] init];
//    lab.text =[dataArray objectAtIndex:index];
//    lab.frame = CGRectMake(0, 0,320,490);
//    
//}
@end
