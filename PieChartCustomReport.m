...
@interface PieChartCustomReport ()
@property (strong) NSArray *objectCat;

@end

@implementation PieChartCustomReport
- (void)viewDidLoad {
    [super viewDidLoad];
...
-(int)amountAgregator:(Category *)category{
    ...
    if (listItemsFromCategory && [listItemsFromCategory count] ) {
        ...
        for (Item *item in listItemsFromCategory) {
            ...
            _dateFrom = [dateFormater dateFromString:[Globals loadSaveDataFrom] ];
            
            NSLog(@"Datumot e %@" , _dateFrom);
            
            
            _dateTo = [dateFormater dateFromString:[Globals loadSaveDataTo]];
            
            // NSDate * itemdate = [dateFormater1 dateFromString:itemDateString];
            
            NSLog(@"Datumot e %@" , _dateTo);
            
            _dateString = [dateFormater stringFromDate:item.date];
            
            NSDate * dateitem= [dateFormater dateFromString:_dateString];
